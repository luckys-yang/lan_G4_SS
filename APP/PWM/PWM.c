...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	if(htim==(&htim2))
	{
		if(htim->Channel==HAL_TIM_ACTIVE_CHANNEL_1)
		{
			...
			{
				TIM_IC_flag++;
				TIM_IC_2.TIM_ONE = TIM2->CCR1;	//记录第一次上升沿
				__HAL_TIM_SET_CAPTUREPOLARITY(&htim2,TIM_CHANNEL_1,TIM_INPUTCHANNELPOLARITY_FALLING);	//下降沿捕获
			}
			else if(2 == TIM_IC_flag)
			{
				TIM_IC_flag++;
				TIM_IC_2.TIM_TWO = TIM2->CCR1;	//记录第一次下降沿
				__HAL_TIM_SET_CAPTUREPOLARITY(&htim2,TIM_CHANNEL_1,TIM_INPUTCHANNELPOLARITY_RISING);	//上升沿捕获			
			}
			else if(3 == TIM_IC_flag)
			{
				TIM_IC_flag++;
				TIM_IC_2.TIM_THREE = TIM2->CCR1;	//记录第二次上升沿
				__HAL_TIM_SET_CAPTUREPOLARITY(&htim2,TIM_CHANNEL_1,TIM_INPUTCHANNELPOLARITY_FALLING);	//下降沿捕获
			}
			else if(4 == TIM_IC_flag)
			{
				TIM_IC_flag++;
				TIM_IC_2.TIM_FOUR = TIM2->CCR1;	//记录第二次下降沿
