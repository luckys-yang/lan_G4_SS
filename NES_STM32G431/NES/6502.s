...
;----------------------------------------------------------------------------
	...
	ldr r0,=0x48000410 ; 端口输入数据寄存器(GPIOB_IDR)0x48000410	按键端口
	ldr r0,[r0]		  ;	   按下为0
				    
	and r0,r0,#0x07		;保留低3位 即 PB0 1 2
	rsb r0,r0,#0x07   ; R0 = #0x07-R0      反向减法?   取反		  按下为1

	and r2,r2,#0x0 ;	R2清零
	and r1,r0,#0x01   ;	B1按下
	orr r2,r1,lsl#3 ;	Start
	...
	ldr r0,=0x48000010 ; 端口输入数据寄存器(GPIOA_IDR)0x48000010	按键端口
	ldr r0,[r0]		  ;	

	and r0,r0,#0x01;	保留低1位 即 PA0  
	rsb r0,r0,#0x01   ;	// 
