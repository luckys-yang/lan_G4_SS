...
//汇编函数定义
//extern void nop6502(void);
//extern int get6502memory(WORD addr); //读6502存储器	
...
//Non maskerable interrupt 非maskerable中断 
void nmi6502(void)
{
	put6502memory(0x0100+S--,(BYTE)(PC>>8));
	put6502memory(0x0100+S--,(BYTE)(PC & 0xff));
...
// Maskerable Interrupt  maskerable中断
void irq6502(void)
{
	put6502memory(0x0100+S--,(BYTE)(PC>>8));
	put6502memory(0x0100+S--,(BYTE)(PC & 0xff));
...
void init6502mem( u8* exp_romptr,
				  u8* sramptr,			
				  u8* prg_rombank0ptr,	//rom_file[16]	
			   	  u8  rom_num)		
{
	exp_rom		= exp_romptr;							//expansion rom 扩展ROM
	sram		= sramptr;								//sram 
	prg_rombank0= prg_rombank0ptr;						//prg-rom lower bank  
	prg_rombank1= prg_rombank0ptr +(0x4000*(rom_num-1));// prg-rom upper bank PRG-ROM上银行
