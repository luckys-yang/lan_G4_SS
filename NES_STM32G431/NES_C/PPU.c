...
SpriteType  *sprite = (SpriteType*)&spr_ram[0]; //指向第一个sprite 0 的位置

// 显示相关 
u8 	SpriteHitFlag, PPU_Latch_Flag; 	//sprite #0 显示碰撞点所在扫描行号, 背景位移￥2005写入标志 
int	PPU_scanline;  					//当前扫描行
u16	Buffer_scanline[(8+256+8)*2];				//[8 + 256 + 8];	//行显示缓存,上下标越界最大为7，显示区 7 ~ 263  0~7 263~270 为防止溢出区
...
//写PPU寄存器		 
void PPU_RegWrite(u8 value,u16 RX)
{			    
	switch(RX)	 //r12  addr&0x7
	{
		//$2000 
...
//？前面表逻辑条件，：前面也就是？后面表示条件成立时的值，：后面表条件不成立时的值。
...
void NES_RenderLine(int y_axes)
{
	...
	}else for(i=8; i<264; i++)Buffer_scanline[i] = 0x0000;//清空显示缓存,黑屏
	//完成扫描，将行显示缓存写入LCD*/
...
void NES_LCD_DisplayLine(int y_axes, u16 *Disaplyline_buffer)
{
	...
	LCD_SetCursor(y_axes, 0); 	//设置光标
	LCD_WriteRAM_Prepare();		//准备写入LCD RAM
	GPIOB->BRR  |=  GPIO_PIN_9; //片选 
	GPIOB->BSRR |=  GPIO_PIN_8; //写

	for(index = 8; index < 264; index++)
		//原始数据16		256
	{
		GPIOB->BSRR |=  GPIO_PIN_5;
		GPIOC->ODR = Buffer_scanline[index];
